...
(chemicalml 0.3.0+). The mapping is one-way in production: the
adapter produces native wire objects; the CML Translator enriches
with aci: extensions only for backwards-compat fallback.

[cols="1,1,2", options="header"]
|===
| Model class | Wire container | Native child elements
| Crystal |  |  with  cells, ,  (fractional coords)
| Spectrum |  |  with  of  (xValue/yValue/yMultiplicity/title)
| ZMatrix |  |  (text content; structural length/angle/torsion children are future work)
| Calculation |  |  of  with  (value, units)
| Mechanism |  |  (text via title; structural step list is future work)
| Reaction conditions |  |  with  (above/below)
|===

== Extension channels (six parallel modules)

Each channel is a MECE module under :

*  — per-atom aci: attributes (FIELDS registry)
*  — top-level constructs via aci: elements (HANDLERS registry)
*  —  inside molecules (BRACKETS registry on Group)
*  — preserves unknown top-level elements verbatim
*  — per-molecule key/value metadata (aci:meta-*)
*  — now mostly legacy; native <conditionList> preferred

The  is a thin orchestrator (~80 lines) that calls each
module's // in pipeline order.

== Model-owned wire codes
...
*  / 
*  /  /  / 
*  (shared by adapter and extension modules)
...
*  — charge conservation (v0.7+)
*  — group bracket consistency
*  — cell lengths/angles/fractional coords (v0.6+)
*  — forward references and geometry (v0.6+)
*  — peak data validation (v0.6+)
*  — periodic-table membership
*  — isotope mass >= atomic number
*  — unmatched ring-closure digits
*  — bond order + charge <= max valence

 sorts by severity then deduplicates.
...
| Crystal
| `<molecule>`
| `<crystal>` with `<scalar>` cells, `<symmetry>`, `<atomArray>` (fractional coords)
...
| Calculation
| `<molecule>`
| `<propertyList>` of `<property>` with `<scalar>` (value, units)
