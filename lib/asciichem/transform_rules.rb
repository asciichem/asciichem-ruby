...
module AsciiChem
  # The transform's rules, backend-neutral: included by Transform
  # (parslet) and by Engine::ParsanolEngine::Transform when that
  # opt-in engine is active. String class_eval so the nested builder
  # classes land as constants of the including class, unchanged.
  module TransformRules
    def self.included(base)
      base.class_eval <<~'RULES'
        # frozen_string_literal: true
      ...
      class CascadeBuilder
        ...
        def canonicalise(data)
          # The grammar wraps the whole leg repeat in one :segments
          # capture (electron-config pattern), which every engine
          # arrays; parslet additionally arrays the first/segments
          # pair. Both reduce to [first, [[arrow, products], ...]].
          hash = data.is_a?(Array) ? data.find { |s| s.is_a?(Hash) && s.key?(:first) } : data
          first = hash[:first]
          [first, wrap(hash[:segments]).map { |seg| [seg[:arrow], seg[:products]] }]
        end
